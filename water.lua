...
function water.spawnBubble(gameState)
  ...
  for _, waterArea in ipairs(gameState.water or {}) do
    if love.math.random() < 0.3 then -- 30% chance per water area
      local bubble = {
        ...
        life = 3 + love.math.random() * 2,
        alpha = 0.3 + love.math.random() * 0.4
      }
...
function water.draw(gameState, globalTime)
  ...
  for _, waterArea in ipairs(gameState.water) do
    -- Main water body
    love.graphics.setColor(0.1, 0.3, 0.6, 0.8) -- Dark blue water
