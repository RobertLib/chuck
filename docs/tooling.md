...
- **`--screen NAME` is the answer**, and it is a switch rather than synthesised
  ...
  `resume`, `restroom`, `aftermath`, `cover` —
...
`make press` photographs the game — nineteen stills, five animations, a cover at
the size itch.io asks for, the page banner and a wallpaper, into `dist/press/`
with a
`MANIFEST.txt` saying what each file is and the command that produced it. (The
pair of counts is derived by `check_docs.py` from the script's own `still` and
`animation` calls; the last hand-written pair on this page said twenty against a
script that takes nineteen.)

**The banner is the same frame cut wide**, and the one thing to know about it is
that itch.io's theme editor makes it *replace the page's title*. So the wordmark
in it is not decoration: it is the only place the game's name appears above the
description, which is why the banner is cut from the cover rather than from a
floor — a band of corridor with no logotype in it would take the title off the
...
Actions tab that hands one back, and it exists for that reason alone. The web one
cross-builds anywhere emscripten runs, which is everywhere, because the platform
it targets is a browser rather than an operating system.

**The library travels in the payload on all four.** SDL3 is new enough that most
...
**And the web one is the exception to the sentence above, which is the one thing
about it worth reading twice.** itch.io looks for `index.html` at the *root* of an
HTML5 zip and offers no way to say otherwise, so the payload every one of its
siblings deliberately nests is the payload this one must not: nested, it is a page
that does not start and says nothing about why. `build_web.sh` zips from inside the
staging directory rather than above it, and the note saying so sits at the top of
the script, because it reads like a mistake against three files that do the
opposite.
...
signature on the platform that refuses to start those. The web archive has none
either, and for the plainest reason of the three: there is no app, no choice, and
one page.
...
`make web` writes `dist/Chuck-<v>-web.zip`: `index.html`, `chuck.js`,
`chuck.wasm`, and nothing else. 604KB, because the design decision this whole
repository is built on — every pixel drawn at runtime, every sound synthesised at
startup, the levels compiled into the binary — turns out to be the thing that
makes a web port small. There is no asset bundle to download because there are no
assets.

**Almost none of it was a code change, and that is the interesting part.** The
game already used [SDL's main callbacks](https://wiki.libsdl.org/SDL3/README-main-functions),
so there was no `while (running)` loop to turn inside out for a platform that
owns the frame; `SDL_AppIterate` *is* the `requestAnimationFrame` callback, and
`SDL_SetRenderVSync(renderer, 1)` — which was already there, for a reason about
pacing that has nothing to do with browsers — is what SDL's emscripten
documentation asks for on the next page. The whole tree compiled to WebAssembly
under `-Wall -Wextra -Wpedantic` on the first attempt with nought warnings, which
is the third compiler to say so.

**The saves needed no code at all, and they very nearly needed a great deal.** A
browser tab's filesystem is RAM: `SDL_GetPrefPath` hands back a path, the game
writes `settings.cfg` and `progress.cfg` into it exactly as it does everywhere
else, the write *succeeds*, and then the tab closes and the disk stops existing.
That is the worst shape this failure could have — nothing reports an error,
because nothing has gone wrong except the ground. SDL solves it as a build option
of its own: `-DSDL_EMSCRIPTEN_PERSISTENT_PATH=/storage` mounts the browser's
IndexedDB there, waits for it to sync *before* calling `SDL_AppInit`, and roots
`SDL_GetPrefPath` underneath it. So the six call sites in
[game.c](../src/game.c) that ask `pref_file_path` for a path are untouched.
This is why SDL is built from source here while the other three payloads fetch a
published binary: what a wasm player's settings survive is decided at SDL's
compile time, and no published build could decide it for us.

It is checked rather than assumed, in the one place a build option can leave a
trace: SDL's own `sdl3.pc` gains `-lidbfs.js` when it has been configured this
way, and `build_web.sh` greps for it before linking. A stale `build/sdl-web` from
an earlier invocation would otherwise link silently into a game that saves
nothing and says so nowhere.

**The one thing that had to change is the way out.** Everywhere the game ships as
a file somebody downloaded, the title screen's last chip is QUIT and holding it
closes the process. A browser tab has no process to close: cancelling the main
loop stops the frame and leaves the canvas standing there with the last picture
frozen on it, which reads as a crash rather than as having been asked for. The
tab is the way out and the player already has it, so `CHUCK_OFFERS_QUIT` is
nought under emscripten and the chip is not drawn, hovered, hit-tested or held —
read the same way `resume_offered` is read, and for the same reason it is a flag
rather than four `#ifdef`s. The pause sheet needs none of this: ABANDON RUN
returns to the title screen, which is something a browser does perfectly well.

**The page is not the one emscripten generates.** `packaging/web_shell.html` is,
and three things in it are load-bearing rather than decoration. The canvas carries
no border and no padding, because SDL's README says so in as many words and the
mouse coordinates it computes come off the element's box. The canvas fills its
frame and SDL does the scaling. And the game does not start on load. `-sINVOKE_RUN=0`
holds `main` back until the PLAY button is pressed, so that the first frame the
player sees is also the first frame allowed to make a sound — a browser throws
audio away until the page has been interacted with, and SDL will open a device and
feed a muted one for as long as that lasts, which is a title screen whose attract
music never arrives.

**And the scaling was got wrong once, in the direction that looks like care.**
The first version of this page kept the canvas backing store at `VIEW_W`x`VIEW_H`
and scaled the *CSS* box by whole numbers, on the argument this repository makes
everywhere else: a non-integer resize of pixel art either blurs it or puts uneven
stems on the letters. Measured, the backing store did not stay anything of the
kind — SDL creates the window `SDL_WINDOW_RESIZABLE`, and its emscripten backend
answers that by keeping the backing store equal to the element's CSS size, so
what the page was actually choosing was the size SDL rendered at, and the
scaling was SDL's letterbox all along.

Which is worth writing down twice over. The comment was wrong about *who* was
scaling, and a comment that explains a mechanism the code does not have is the
defect this file's own conventions page spends pages on. And the rule it was
enforcing was stricter than the game's: on a 1920x1080 fullscreen, the largest
whole number that fits `800x552` is 1, so the integer rule drew the game at 1:1
in the middle of a black field, while the same fullscreen on a desktop letterboxes
at 1.956 and fills the screen — `SDL_LOGICAL_PRESENTATION_LETTERBOX`, chosen in
[game.c](../src/game.c) for that exact reason. **A web build that is more careful
than the game is not more careful, it is different**, and different is the thing a
port must not be. The canvas fills its frame now and SDL scales it, which at the
800x552 viewport itch.io is told to use is no scaling at all. Measured at four
frame sizes: 800x552 comes back 1:1, 1600x1104 comes back exactly 2x, and
1920x1080 fills the screen.

**There are no threads and that is a decision.** Threads on the web need COOP/COEP
headers set by the server, itch.io does not set them, and a build that wants them
does not run slowly there — it does not start. Nothing under `src/` creates a
thread; the one `SDL_CreateThread` in this repository is the editor's build
button, and the editor is not a thing a player is handed.

### What was measured, and what is not gated

`make test` links no SDL and `make lint` reads text, so **neither can see this
build at all** — which is the same sentence this repository already writes about
the renderers, one platform further out. What can be said about it is what was
actually run, and it is written here rather than claimed:

- **It renders.** The game's own `--shot` was driven inside headless Chrome and
  the BMP pulled back out of the virtual filesystem: `--screen resume` and
  `--screen aftermath` both came back at 1766538 bytes, the same size the native
  build writes, showing the title screen and sector 9's archive hall with the
  HUD, the bodies, the alarm lighting and the opened patch on it. Which is the
  same trick [the soak sweep](#the-headless-soak) plays natively, and it works
  because `--shot` is a switch rather than a menu.
- **The shipped page works, PLAY button and all.** Chrome was driven over the
  DevTools protocol against the unpacked archive: the button enables when the
  wasm finishes downloading, the click starts the run, the canvas ends up
  800x552 backing at 800x552 CSS in a 1000x700 window, and the frame photographed
  afterwards is the title screen with the QUIT chip correctly absent and the
  remaining chips re-centred.
- **The saves survive a reload.** A file written into `/storage/rob/Chuck` — the
  exact directory `SDL_GetPrefPath(CHUCK_APP_ORG, CHUCK_APP_NAME)` returns — read
  back in the next page load of the same profile, and `/storage` reports itself
  as an IDBFS mount. Worth knowing what the same probe showed on the way past: a
  run started with `--soak` created no such directory, because `--soak` makes the
  run a *scripted* one and `pref_file_path` refuses those. The rule this tree
  already had works on the web without having been told about it.

What is **not** covered: none of the above runs in CI, and there is no gate that
would notice the browser build breaking. Adding one means a browser and a web
server on a runner, which is a real cost, and pretending otherwise by adding a
check that only compiles the wasm would be a check reporting coverage it does not
have. The honest position is that this is a manual verification with the commands
written down, and that the compile itself — which `make web` does do — is the part
that catches most of what actually breaks.
