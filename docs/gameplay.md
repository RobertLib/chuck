...
answers away. It is the stomp, and the choice is not arbitrary: it costs no
ammunition and no position, and it is what anybody reaches for the moment a
floor gets busy. `enemy_kind_can_be_stomped` is the whole of the rule in
`gameplay_combat_check_contacts`; landing on him costs the heart a side contact
costs, which is what the existing branch already does.

This paragraph called it the ***free*** one until a guard was finally wired to
shoot at where the boot came from — see
[the shot back](#the-shot-a-stomped-guard-cannot-miss-with) below — and the
...
### The shot a stomped guard cannot miss with

A stomp calls `damage_enemy`, which calls `gameplay_provoke_enemy`, which starts
the aim telegraph — so a guard has *answered* every stomp since the day the
stomp existed. He simply could not connect. That function wrote `aim_vdir = 0`
flat, and `fire_enemy_bullet` clamps a horizontal round into the guard's own
`ENEMY_MUZZLE_MIN_Y_FACTOR`..`MAX` band whatever the aim said, so the round left
the muzzle at chest height while the boot that provoked it was two tiles over
his helmet.

Measured on a flat floor with one guard: **he fired twice inside the 1.5 seconds
a three-stomp kill takes, at 43px under the player's own boots, and the kill
cost nought hearts.** The telegraph was drawn on every one of those frames, which
is the whole reason nobody had noticed — **a response aimed at the one place the
attack cannot come from is worse than no response at all**, because the picture
says otherwise and the player learns that jumping on heads is free.

`gameplay_ai_aim_at_player` ([gameplay_ai.c](../src/gameplay_ai.c)) is the axis
and the aim together, and both places a man begins one now go through it — the
combat pass off a firing solution, the provoke off having been hit. It asks
nothing about line of sight, and that is the difference between the two callers
rather than an omission: being shot, knifed, crated or landed on has already
answered that question.

**What decides "roughly level" is the muzzle clamp, not a distance**, and that is
why the first draft of the fix changed nothing at all. Borrowing
`enemy_shot_solution`'s own `TILE_SIZE * 1.2f` preference band puts a man
standing on the guard's *head* inside it: the band is 38px and a body is 32
tall, so the two centres sit 32 apart and read as level. Asking the clamp
instead asks the thing that actually makes the shot miss — if the player's box
does not reach the strip a horizontal round can occupy, a shot down the corridor
is a miss before the trigger is pulled. Outside the strip and outside
...
alone. So the cheapest way to stomp a man to death is still to commit to it,
which is the right shape — the mechanic charges for the kill rather than
punishing the player for hesitating.

`test_a_stomped_guard_shoots_at_the_man_on_his_head` holds the axis, the
consequence and the confinement. All three clauses are needed and the middle one
is the only one a count of hearts can see: the rounds were being fired the whole
time this was broken, so a check that only counted them would have passed
throughout.
