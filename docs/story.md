...
**And holding it open is what turned a climb's reveal into three seconds of
nothing.** The animation is what the line rides, so it has to be *visible*, and
on the five walls it was not: `render_facade_world` drew its backdrop and then
returned until the reveal had finished, and the walk itself sweeps the map
row by row while a climb is 25 tiles wide and 41 to 53 tall with the camera on
the bottom row — so the front spent its first two thirds above the top of the
frame. Measured as the longest stretch during which nothing inside the viewport
changes, sectors 3, 7, 11, 13 and 15 ran **2.30 to 2.65 seconds of the 3.80**
against 0.08 to 1.06 on the interiors. A player read the band over a flat wall
and then watched the sector arrive in one frame. The facade branch draws its
structure inside the reveal now, the way the interior branch always has, and the
front sweeps along the map's shorter axis — which leaves every interior's walk
exactly as it was and takes the climbs to 0.11-0.14s. Which is the paragraph
above one axis further over again: the fit checks ask whether a line is too
wide, the hold asks whether it is on the glass long enough, and neither of them
asks whether there is anything behind it to look at.
