...
make press    # photograph the game: 19 stills, 5 GIFs, the itch.io cover and a
              # wallpaper into dist/press/, with a MANIFEST.txt naming the exact
...
**A release is three targets, one per platform, and nothing else.**
...
`make app` still exists and is not one of the three: it builds and signs the
