...
typedef struct
{
    ...
    /* `--screen cover` sets this: the same night drawn as the press kit's
     * key art — tower closer, the cordon on the street, Chuck on the wall,
     * and no interface at all. Nothing in a played game ever sets it; the
     * flag lives here rather than in a state of its own because the cover
     * *is* the title screen with everything turned toward a store page. */
    bool key_art;
    ...
    /* And whether it is there at all. `CHUCK_OFFERS_QUIT` is nought in a
     * browser tab, where there is no process to close; the flag is read the
     * same way `resume_offered` is, so nothing may be hovered, clicked, held
     * or drawn when there is nowhere to quit to. */
    bool quit_offered;
