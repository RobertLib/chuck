...
/* See level.h for what this is for and what it replaced. */
                          ...
                          float factor)
{
    ...
    /* The camera's resting place at the foot of the wall, clamped the way
     * `camera_axis_target` clamps it: a map no taller than the frame never
     * scrolls, and nothing has sunk anywhere. */
    float foot = (float)map->height * (float)TILE_SIZE - view_h;
    if (foot < 0.0f)
        foot = 0.0f;
    float climbed = foot - cam_y;
    if (climbed < 0.0f)
        climbed = 0.0f;
...
void level_reveal_init(Level *level)
{
    ...
    /* See `LevelReveal.by_column`: the front sweeps along the shorter axis,
     * which is the one the camera can hold all of. */
    level->reveal.by_column = level->map.width < level->map.height;
...
bool level_reveal_step(Level *level, float dt)
{
    ...
    while (level->reveal.timer >= level->reveal.interval && !level->reveal.done)
    {
        ...
        {
            /* The walk is over when the front has stepped off the far edge of
             * the axis it travels along, whichever that is. */
            bool walked_off = level->reveal.by_column
                                  ? level->reveal.next_col >= level->map.width
                                  : level->reveal.next_row >= level->map.height;
            if (walked_off)
            {
            ...
            if (level->reveal.next_row < level->map.height &&
                level->reveal.next_col < level->map.width)
            {
                level->reveal.tiles_visible[level->reveal.next_row]
                                           [level->reveal.next_col] = true;
            }
            if (level->reveal.by_column)
            {
                /* Down a column, then one column right. */
