...
typedef struct
{
    ...
    float pane_h;     /* the glazing inside it             */
    float light_odds; /* fraction of panes lit, 0..1       */
    float moon_radius;
} IntroScene;
...
static IntroScene scene_layout(int win_w, int win_h)
{
    ...
    s.floors = TOWER_FLOORS;
    s.panes = TOWER_PANES;
    s.floor_h = TOWER_FLOOR_H;
    s.pane_h = 20.0f;
    /* Sparse on purpose: the title is a building asleep, and the one window
     * that matters has to be the brightest thing on the wall. */
    s.light_odds = 0.30f;
    s.moon_radius = 14.0f;
    return s;
}
...
{
    return s->roof_y + 10.0f + s->floor_h * (float)index;
}
...
static SDL_FRect pane_rect(const IntroScene *s, int fl, int pane)
{
    ...
    float mullion = 5.0f;
    float pane_w = (right - left - mullion * (float)(s->panes - 1)) /
                   (float)s->panes;
 ...
 */
static bool pane_light(const IntroScene *s, int fl, int pane, float time,
                       SDL_Color *out)
{
    unsigned h = fx_hash((unsigned)(fl * 37 + pane * 7 + 11) * 2654435761u);
    if ((h % 100u) >= (unsigned)(s->light_odds * 100.0f + 0.5f))
        return false;
...
static void place_buttons(Intro *intro, int w, int h, const PadHints *pad)
{
    ...
    intro->quit_button.h = intro->quit_offered ? MANUAL_ROW_H : 0.0f;
    ...
    float line_w = intro->manual_button.w + PROMPT_ROW_GAP +
                   intro->options_button.w;
    if (intro->quit_offered)
        line_w += PROMPT_ROW_GAP + intro->quit_button.w;
                ...
                int resume_sector)
{
    ...
    intro->quit_offered = CHUCK_OFFERS_QUIT;
...
{
    return intro->quit_offered && hit_plate(&intro->quit_button, x, y);
}
...
static void draw_moon(SDL_Renderer *r, float cx, float cy, float radius)
{
    ...
    fx_glow(r, cx, cy, 34.0f * k, fx_mix(COL_MOON_DK, COL_MOON, 0.7f), 34);
    ...
    color_rect(r, fx_mix(COL_MOON_DK, COL_MOON, 0.25f), cx - 6.0f * k,
               cy - 5.0f * k, 5.0f * k, 4.0f * k);
    color_rect(r, fx_mix(COL_MOON_DK, COL_MOON, 0.25f), cx + 3.0f * k,
               cy + 6.0f * k, 4.0f * k, 3.0f * k);
    color_rect(r, fx_mix(COL_MOON_DK, COL_MOON, 0.45f), cx - 2.0f * k,
               cy + 2.0f * k, 3.0f * k, 2.0f * k);
    color_rect(r, fx_mix(COL_MOON_DK, COL_MOON, 0.35f), cx + 6.0f * k,
               cy - 2.0f * k, 2.0f * k, 2.0f * k);
}
...
static void render_sky(SDL_Renderer *r, const Intro *intro, const IntroScene *s)
{
    ...
    draw_moon(r, s->w * 0.135f, 88.0f, s->moon_radius);
                           ...
                           const IntroScene *s)
{
    ...
    /* Planted relative to the pavement, so a layout with a lower street keeps
     * the rows standing on it: 406 and 442 on the title screen, unchanged. */
    const float far_base = s->street_y - 54.0f;
    int cursor = -24;
    ...
    const float base = s->street_y - 18.0f;
    cursor = -10;
    for (int i = 0; (float)cursor < s->w + 30.0f; ++i)
    {
...
static void draw_lobby(SDL_Renderer *r, const IntroScene *s, float time,
                       float wake)
{
    float top = floor_top(s, s->floors);
                         ...
                         const IntroScene *s)
{
    ...
    for (int i = 0; i < s->floors; ++i)
    {
        ...
        color_rect(r, COL_VOID, left, top, right - left, s->floor_h);
        ...
        for (int pane = 0; pane < s->panes; ++pane)
        {
            ...
            if (i == TARGET_FLOOR && pane == TARGET_PANE)
                continue;
            if (pane_light(s, i, pane, time, &light))
            {
        ...
        for (int pane = 0; pane <= s->panes; ++pane)
        {
            SDL_FRect p = pane_rect(s, i, pane < s->panes ? pane : pane - 1);
            float px = pane < s->panes ? p.x - 5.0f : p.x + p.w;
            float pw = pane < s->panes ? 5.0f : 5.0f;
            if (pane == 0)
            {
                px = left;
                pw = p.x - left;
            }
            else if (pane == s->panes)
            {
                px = p.x + p.w;
                pw = right - px;
            }
...
static void draw_wet_reflections(SDL_Renderer *r, const IntroScene *s,
                                 float time)
{
    float y = s->street_y;
    ...
    for (int i = 0; i < s->floors; ++i)
    {
        for (int pane = 0; pane < s->panes; ++pane)
        {
            ...
            if (!target && !pane_light(s, i, pane, time, &light))
                continue;
            if (target)
                light = COL_TARGET;
...
static void draw_lobby_spill(SDL_Renderer *r, const IntroScene *s, float wake)
{
    float y = s->street_y;
     ...
     */
    float lobby_top = floor_top(s, s->floors);
...
static void draw_ground_fog(SDL_Renderer *r, const IntroScene *s, float time)
{
    float y = s->street_y;
                                 ...
                                 const IntroScene *s, const PadHints *pad)
{
    ...
    render_prompt_chip(r, &intro->options_button, appear,
                       ...
                       OPTIONS_LABEL);
    /* And the last chip only exists where there is something to quit to. */
    if (intro->quit_offered)
        render_prompt_chip(r, &intro->quit_button, appear,
                           intro->quit_hovered || intro->quit_hold > 0.0f,
                           pad_hint(pad, quit_key, sizeof(quit_key), "$B",
                                    "ESC"),
                           QUIT_LABEL);
