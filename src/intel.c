 ...
 * up sector 16, which now answers it with the mechanism: the door takes two
...
static const char *const TRANSITION_INTEL[] = {
     ...
     *
     * Nothing caught it because the row is one of the eleven a window
     * suppresses. The suite measures every row's *width* and `INTEL_ARC_SECTORS`
     * pins which of them a player reaches, so this line was checked for the one
     * property that does not matter until a map gives it back. `check_docs.py`
     ...
     * from one — so this row is written and never read. It says a penthouse
     * thing rather than an arc thing for exactly that reason: the beat it used
     * to carry is at 16 now, where the player will actually see it.
     */
