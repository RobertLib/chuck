 ...
 *
 * Everywhere the game ships as a file somebody downloaded, it does: the title
 * screen's last chip is QUIT and holding it closes the process. In a browser
 * tab there is no process to close and no desktop to be handed back to —
 * `emscripten_cancel_main_loop` stops the frame and leaves the canvas standing
 * there with the last picture frozen on it, which reads as a crash rather than
 * as having been asked for. The tab is the way out, and the player already has
 * it.
 *
 * It is a `#define` here rather than an `#ifdef __EMSCRIPTEN__` at the four
 * places that would need one, for the reason `resume_offered` is a flag rather
 * than a zero-width rect: what the chip costs is a hover, a hit test, a hold
 * and a drawing, and a platform test repeated at each of them is four places
 * to get it wrong. The pause sheet needs none of this — ABANDON RUN returns to
 * the title screen, which is a thing a browser can do perfectly well.
 */
#ifdef __EMSCRIPTEN__
#define CHUCK_OFFERS_QUIT 0
#else
#define CHUCK_OFFERS_QUIT 1
#endif
 ...
 * harder — **the stomp**, the one that costs no ammunition and no position,
 * and the one a player falls back on the moment a floor gets busy.
 *
 * **It used to be the *free* one, and this comment said so in that word.** A
 * stomp provokes, a provoke starts the aim telegraph, and `aim_vdir` was
 * written flat — so a guard answered the boot on his helmet with a round
 * clamped to chest height, 43px under the player's own boots, twice inside the
 * 1.5 seconds a three-stomp kill takes. He responded on every frame the player
 * could see the telegraph and connected on none of them, which is why nobody
 * had noticed he was not defending himself. Wired to the axis the player is
 * actually on, the same kill costs a heart of three; see
 * `gameplay_ai_aim_at_player`.
 *
 * That makes the heavy's own rule *cleaner* rather than weaker, which is worth
 * saying because it is the reverse of what a nerf to the stomp usually does to
