...
def main() -> int:
    ...
    phrase_checks = [
        ...
        # The same pair, on the tooling page and in the script's own header —
        # both said twenty for a release after the count above was corrected,
        # which is the fix-lands-on-one-copy defect this file exists for.
        (
            "the press kit's inventory, on its own page",
            DOCS / "tooling.md",
            "photographs the game",
            [
                f"{spelled(press_stills)} stills, "
                f"{spelled(press_gifs)} animations",
            ],
        ),
        (
            "the press kit's inventory, in its own header",
            ROOT / "tools" / "press_kit.sh",
            "Photograph the game",
            [
                f"{spelled(press_stills)} stills, "
                f"{spelled(press_gifs)} animations",
