#!/bin/bash
#
# Photograph the game: nineteen stills, five animations, the itch.io cover and
# a wallpaper, into dist/press/ with a MANIFEST.txt naming what each file is.
# (The counts are held to the `still`/`animation` calls below by
# tools/check_docs.py — the last hand-written pair here was stale by one.)
#
...
# ---------------------------------------------------------------------------
if [ -n "$im" ]; then
    if capture cover --shot-at 3.0 --screen cover; then
        "$im" "$work/cover.bmp" -crop 696x552+52+0 +repage \
        ...
        printf '%-28s %s\n%-28s   $ chuck --seed %s --shot cover.bmp --shot-at 3.0 --screen cover  # cropped 696x552+52+0\n\n' \
            "cover-630x500.png" \
            "itch.io cover art, from the key-art screen (and @2x)" \
            "" "$seed" >>"$manifest"
        echo "press: cover-630x500.png"

        # The banner, off the same frame. itch.io's theme editor takes one, it
        # displays at 960 wide, and the thing to know about it is that it
        # **replaces the page's title** — so the wordmark is not decoration here,
        # it is the only place the game's name appears above the description.
        ...
        printf '%-28s %s\n%-28s   $ chuck --seed %s --shot cover.bmp --shot-at 3.0 --screen cover  # cropped 800x300+0+12\n\n' \
            "banner-960x360.png" \
            "itch.io page banner (Edit theme); it replaces the page title" \
            "" "$seed" >>"$manifest"
        echo "press: banner-960x360.png"
    fi
fi
...
if [ -n "$im" ] && [ -f "$work/18-title.bmp" ]; then
    ...
    # band with the sky above the tower kept and the pavement trimmed. Still the
    # title rather than the cover on purpose — a page background sits behind
    # store text, and the calmer of the two shots is the one that belongs there.
