...
`make win` and `make linux` are the other two downloads, and `make web` is the
fourth release: `dist/Chuck-<version>-web.zip`, the game compiled to WebAssembly
so it plays in a browser tab, which is how most of itch.io is actually played.
Those are the four — one target per platform, each writing one archive into
`dist/` and stopping there. The web one needs
[emscripten](https://emscripten.org/) and cross-builds anywhere that runs; it
carries no external files either, so the whole game is about six hundred
